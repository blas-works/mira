...
export function setupAutoUpdater(mainWindow: BrowserWindow | null, store: StoreInterface): void {
  if (is.dev) return
...
export function checkPendingUpdate(): void {
  ...
  const pending = storeRef.get('pendingUpdate') as PendingUpdate | null
  if (!pending) return
